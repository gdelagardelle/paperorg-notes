...
final class OpenAITranscriptionProvider: TranscriptionProvider, @unchecked Sendable {
    ...
    func transcribe(_ request: TranscriptionRequest, credentials: TranscriptionCredentials) async throws -> TranscriptionResult {
        ...
        let parsed = try JSONDecoder().decode(OpenAIVerboseResponse.self, from: data)
        let segments = parsed.segments?.enumerated().map { index, seg in
...
private struct OpenAIVerboseResponse: Decodable {
    let text: String
    let duration: Double?
